...
test('회의록 섹션과 자료 저장소(드라이브) 섹션이 표시된다', async () => {
  ...
  const refLink = screen.getByRole('link', { name: /참고 자료 폴더 열기/ })
  expect(refLink).toHaveAttribute('href', 'https://drive.google.com/drive/folders/18je1FtxZURnp8AN22tO2Bjk3hHfC4ljn')
  expect(refLink).toHaveAttribute('target', '_blank')
  const msLink = screen.getByRole('link', { name: /원고 업로드 폴더 열기/ })
  expect(msLink).toHaveAttribute('href', 'https://drive.google.com/drive/folders/1ksWD3v9aeOA-FpTTNf2FK-qIhQKjkP2i')
  expect(msLink).toHaveAttribute('target', '_blank')
  const dbLink = screen.getByRole('link', { name: /교과서 문학 작품 DB 열기/ })
  expect(dbLink).toHaveAttribute('href', 'https://merciybr-cmyk.github.io/literature-db/')
  expect(dbLink).toHaveAttribute('target', '_blank')
